...
export default function MyAccount() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // 実際のアプリケーションでは、ここでAPIエンドポイントからユーザー情報を取得します
        // 例: const response = await fetch('/api/user');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 実際のアプリケーションでは、ここでAPIエンドポイントにユーザー情報を送信します
      // 例: const response = await fetch('/api/user', { method: 'PUT', body: JSON.stringify(formData) });
      
      // デモ用の処理
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      setUser(formData as UserInfo);
      setIsEditing(false);
      setSaveSuccess(true);
